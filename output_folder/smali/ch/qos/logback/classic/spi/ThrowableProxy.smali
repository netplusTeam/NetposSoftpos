.class public Lch/qos/logback/classic/spi/ThrowableProxy;
.super Ljava/lang/Object;
.source "ThrowableProxy.java"

# interfaces
.implements Lch/qos/logback/classic/spi/IThrowableProxy;


# static fields
.field private static final GET_SUPPRESSED_METHOD:Ljava/lang/reflect/Method;

.field private static final NO_SUPPRESSED:[Lch/qos/logback/classic/spi/ThrowableProxy;


# instance fields
.field private calculatedPackageData:Z

.field private cause:Lch/qos/logback/classic/spi/ThrowableProxy;

.field private className:Ljava/lang/String;

.field commonFrames:I

.field private message:Ljava/lang/String;

.field private transient packagingDataCalculator:Lch/qos/logback/classic/spi/PackagingDataCalculator;

.field stackTraceElementProxyArray:[Lch/qos/logback/classic/spi/StackTraceElementProxy;

.field private suppressed:[Lch/qos/logback/classic/spi/ThrowableProxy;

.field private throwable:Ljava/lang/Throwable;


# direct methods
.method static constructor <clinit>()V
    .locals 5

    .line 39
    const/4 v0, 0x0

    .line 41
    .local v0, "method":Ljava/lang/reflect/Method;
    const/4 v1, 0x0

    :try_start_0
    const-class v2, Ljava/lang/Throwable;

    const-string v3, "getSuppressed"

    new-array v4, v1, [Ljava/lang/Class;

    invoke-virtual {v2, v3, v4}, Ljava/lang/Class;->getMethod(Ljava/lang/String;[Ljava/lang/Class;)Ljava/lang/reflect/Method;

    move-result-object v2
    :try_end_0
    .catch Ljava/lang/NoSuchMethodException; {:try_start_0 .. :try_end_0} :catch_0

    move-object v0, v2

    .line 44
    goto :goto_0

    .line 42
    :catch_0
    move-exception v2

    .line 45
    :goto_0
    sput-object v0, Lch/qos/logback/classic/spi/ThrowableProxy;->GET_SUPPRESSED_METHOD:Ljava/lang/reflect/Method;

    .line 48
    .end local v0    # "method":Ljava/lang/reflect/Method;
    new-array v0, v1, [Lch/qos/logback/classic/spi/ThrowableProxy;

    sput-object v0, Lch/qos/logback/classic/spi/ThrowableProxy;->NO_SUPPRESSED:[Lch/qos/logback/classic/spi/ThrowableProxy;

    return-void
.end method

.method public constructor <init>(Ljava/lang/Throwable;)V
    .locals 7
    .param p1, "throwable"    # Ljava/lang/Throwable;

    .line 50
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    .line 31
    sget-object v0, Lch/qos/logback/classic/spi/ThrowableProxy;->NO_SUPPRESSED:[Lch/qos/logback/classic/spi/ThrowableProxy;

    iput-object v0, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->suppressed:[Lch/qos/logback/classic/spi/ThrowableProxy;

    .line 34
    const/4 v0, 0x0

    iput-boolean v0, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->calculatedPackageData:Z

    .line 52
    iput-object p1, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->throwable:Ljava/lang/Throwable;

    .line 53
    invoke-virtual {p1}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    move-result-object v1

    invoke-virtual {v1}, Ljava/lang/Class;->getName()Ljava/lang/String;

    move-result-object v1

    iput-object v1, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->className:Ljava/lang/String;

    .line 54
    invoke-virtual {p1}, Ljava/lang/Throwable;->getMessage()Ljava/lang/String;

    move-result-object v1

    iput-object v1, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->message:Ljava/lang/String;

    .line 55
    invoke-virtual {p1}, Ljava/lang/Throwable;->getStackTrace()[Ljava/lang/StackTraceElement;

    move-result-object v1

    invoke-static {v1}, Lch/qos/logback/classic/spi/ThrowableProxyUtil;->steArrayToStepArray([Ljava/lang/StackTraceElement;)[Lch/qos/logback/classic/spi/StackTraceElementProxy;

    move-result-object v1

    iput-object v1, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->stackTraceElementProxyArray:[Lch/qos/logback/classic/spi/StackTraceElementProxy;

    .line 57
    invoke-virtual {p1}, Ljava/lang/Throwable;->getCause()Ljava/lang/Throwable;

    move-result-object v1

    .line 59
    .local v1, "nested":Ljava/lang/Throwable;
    if-eqz v1, :cond_0

    .line 60
    new-instance v2, Lch/qos/logback/classic/spi/ThrowableProxy;

    invoke-direct {v2, v1}, Lch/qos/logback/classic/spi/ThrowableProxy;-><init>(Ljava/lang/Throwable;)V

    iput-object v2, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->cause:Lch/qos/logback/classic/spi/ThrowableProxy;

    .line 61
    invoke-virtual {v1}, Ljava/lang/Throwable;->getStackTrace()[Ljava/lang/StackTraceElement;

    move-result-object v3

    iget-object v4, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->stackTraceElementProxyArray:[Lch/qos/logback/classic/spi/StackTraceElementProxy;

    invoke-static {v3, v4}, Lch/qos/logback/classic/spi/ThrowableProxyUtil;->findNumberOfCommonFrames([Ljava/lang/StackTraceElement;[Lch/qos/logback/classic/spi/StackTraceElementProxy;)I

    move-result v3

    iput v3, v2, Lch/qos/logback/classic/spi/ThrowableProxy;->commonFrames:I

    .line 63
    :cond_0
    sget-object v2, Lch/qos/logback/classic/spi/ThrowableProxy;->GET_SUPPRESSED_METHOD:Ljava/lang/reflect/Method;

    if-eqz v2, :cond_1

    .line 66
    :try_start_0
    new-array v0, v0, [Ljava/lang/Object;

    invoke-virtual {v2, p1, v0}, Ljava/lang/reflect/Method;->invoke(Ljava/lang/Object;[Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v0

    .line 67
    .local v0, "obj":Ljava/lang/Object;
    instance-of v2, v0, [Ljava/lang/Throwable;

    if-eqz v2, :cond_1

    .line 68
    move-object v2, v0

    check-cast v2, [Ljava/lang/Throwable;

    check-cast v2, [Ljava/lang/Throwable;

    .line 69
    .local v2, "throwableSuppressed":[Ljava/lang/Throwable;
    array-length v3, v2

    if-lez v3, :cond_1

    .line 70
    array-length v3, v2

    new-array v3, v3, [Lch/qos/logback/classic/spi/ThrowableProxy;

    iput-object v3, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->suppressed:[Lch/qos/logback/classic/spi/ThrowableProxy;

    .line 71
    const/4 v3, 0x0

    .local v3, "i":I
    :goto_0
    array-length v4, v2

    if-ge v3, v4, :cond_1

    .line 72
    iget-object v4, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->suppressed:[Lch/qos/logback/classic/spi/ThrowableProxy;

    new-instance v5, Lch/qos/logback/classic/spi/ThrowableProxy;

    aget-object v6, v2, v3

    invoke-direct {v5, v6}, Lch/qos/logback/classic/spi/ThrowableProxy;-><init>(Ljava/lang/Throwable;)V

    aput-object v5, v4, v3

    .line 73
    iget-object v4, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->suppressed:[Lch/qos/logback/classic/spi/ThrowableProxy;

    aget-object v4, v4, v3

    aget-object v5, v2, v3

    invoke-virtual {v5}, Ljava/lang/Throwable;->getStackTrace()[Ljava/lang/StackTraceElement;

    move-result-object v5

    iget-object v6, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->stackTraceElementProxyArray:[Lch/qos/logback/classic/spi/StackTraceElementProxy;

    invoke-static {v5, v6}, Lch/qos/logback/classic/spi/ThrowableProxyUtil;->findNumberOfCommonFrames([Ljava/lang/StackTraceElement;[Lch/qos/logback/classic/spi/StackTraceElementProxy;)I

    move-result v5

    iput v5, v4, Lch/qos/logback/classic/spi/ThrowableProxy;->commonFrames:I
    :try_end_0
    .catch Ljava/lang/IllegalAccessException; {:try_start_0 .. :try_end_0} :catch_1
    .catch Ljava/lang/reflect/InvocationTargetException; {:try_start_0 .. :try_end_0} :catch_0

    .line 71
    add-int/lit8 v3, v3, 0x1

    goto :goto_0

    .line 80
    .end local v0    # "obj":Ljava/lang/Object;
    .end local v2    # "throwableSuppressed":[Ljava/lang/Throwable;
    .end local v3    # "i":I
    :catch_0
    move-exception v0

    goto :goto_1

    .line 78
    :catch_1
    move-exception v0

    .line 85
    :cond_1
    :goto_1
    return-void
.end method


# virtual methods
.method public calculatePackagingData()V
    .locals 2

    .line 136
    iget-boolean v0, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->calculatedPackageData:Z

    if-eqz v0, :cond_0

    .line 137
    return-void

    .line 139
    :cond_0
    invoke-virtual {p0}, Lch/qos/logback/classic/spi/ThrowableProxy;->getPackagingDataCalculator()Lch/qos/logback/classic/spi/PackagingDataCalculator;

    move-result-object v0

    .line 140
    .local v0, "pdc":Lch/qos/logback/classic/spi/PackagingDataCalculator;
    if-eqz v0, :cond_1

    .line 141
    const/4 v1, 0x1

    iput-boolean v1, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->calculatedPackageData:Z

    .line 142
    invoke-virtual {v0, p0}, Lch/qos/logback/classic/spi/PackagingDataCalculator;->calculate(Lch/qos/logback/classic/spi/IThrowableProxy;)V

    .line 144
    :cond_1
    return-void
.end method

.method public fullDump()V
    .locals 7

    .line 147
    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    .line 148
    .local v0, "builder":Ljava/lang/StringBuilder;
    iget-object v1, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->stackTraceElementProxyArray:[Lch/qos/logback/classic/spi/StackTraceElementProxy;

    .local v1, "arr$":[Lch/qos/logback/classic/spi/StackTraceElementProxy;
    array-length v2, v1

    .local v2, "len$":I
    const/4 v3, 0x0

    .local v3, "i$":I
    :goto_0
    if-ge v3, v2, :cond_0

    aget-object v4, v1, v3

    .line 149
    .local v4, "step":Lch/qos/logback/classic/spi/StackTraceElementProxy;
    invoke-virtual {v4}, Lch/qos/logback/classic/spi/StackTraceElementProxy;->toString()Ljava/lang/String;

    move-result-object v5

    .line 150
    .local v5, "string":Ljava/lang/String;
    const/16 v6, 0x9

    invoke-virtual {v0, v6}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    move-result-object v6

    invoke-virtual {v6, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    .line 151
    invoke-static {v0, v4}, Lch/qos/logback/classic/spi/ThrowableProxyUtil;->subjoinPackagingData(Ljava/lang/StringBuilder;Lch/qos/logback/classic/spi/StackTraceElementProxy;)V

    .line 152
    sget-object v6, Lch/qos/logback/core/CoreConstants;->LINE_SEPARATOR:Ljava/lang/String;

    invoke-virtual {v0, v6}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    .line 148
    .end local v4    # "step":Lch/qos/logback/classic/spi/StackTraceElementProxy;
    .end local v5    # "string":Ljava/lang/String;
    add-int/lit8 v3, v3, 0x1

    goto :goto_0

    .line 154
    .end local v1    # "arr$":[Lch/qos/logback/classic/spi/StackTraceElementProxy;
    .end local v2    # "len$":I
    .end local v3    # "i$":I
    :cond_0
    sget-object v1, Ljava/lang/System;->out:Ljava/io/PrintStream;

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v2

    invoke-virtual {v1, v2}, Ljava/io/PrintStream;->println(Ljava/lang/String;)V

    .line 155
    return-void
.end method

.method public getCause()Lch/qos/logback/classic/spi/IThrowableProxy;
    .locals 1

    .line 118
    iget-object v0, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->cause:Lch/qos/logback/classic/spi/ThrowableProxy;

    return-object v0
.end method

.method public getClassName()Ljava/lang/String;
    .locals 1

    .line 101
    iget-object v0, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->className:Ljava/lang/String;

    return-object v0
.end method

.method public getCommonFrames()I
    .locals 1

    .line 109
    iget v0, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->commonFrames:I

    return v0
.end method

.method public getMessage()Ljava/lang/String;
    .locals 1

    .line 92
    iget-object v0, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->message:Ljava/lang/String;

    return-object v0
.end method

.method public getPackagingDataCalculator()Lch/qos/logback/classic/spi/PackagingDataCalculator;
    .locals 1

    .line 129
    iget-object v0, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->throwable:Ljava/lang/Throwable;

    if-eqz v0, :cond_0

    iget-object v0, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->packagingDataCalculator:Lch/qos/logback/classic/spi/PackagingDataCalculator;

    if-nez v0, :cond_0

    .line 130
    new-instance v0, Lch/qos/logback/classic/spi/PackagingDataCalculator;

    invoke-direct {v0}, Lch/qos/logback/classic/spi/PackagingDataCalculator;-><init>()V

    iput-object v0, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->packagingDataCalculator:Lch/qos/logback/classic/spi/PackagingDataCalculator;

    .line 132
    :cond_0
    iget-object v0, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->packagingDataCalculator:Lch/qos/logback/classic/spi/PackagingDataCalculator;

    return-object v0
.end method

.method public getStackTraceElementProxyArray()[Lch/qos/logback/classic/spi/StackTraceElementProxy;
    .locals 1

    .line 105
    iget-object v0, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->stackTraceElementProxyArray:[Lch/qos/logback/classic/spi/StackTraceElementProxy;

    return-object v0
.end method

.method public getSuppressed()[Lch/qos/logback/classic/spi/IThrowableProxy;
    .locals 1

    .line 122
    iget-object v0, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->suppressed:[Lch/qos/logback/classic/spi/ThrowableProxy;

    return-object v0
.end method

.method public getThrowable()Ljava/lang/Throwable;
    .locals 1

    .line 88
    iget-object v0, p0, Lch/qos/logback/classic/spi/ThrowableProxy;->throwable:Ljava/lang/Throwable;

    return-object v0
.end method
