...
class EnterCvvNumberDialog(private val enterCvvListener: PinPadDialogListener) : DialogFragment() {
    ...
    override fun onCreateView(
        ...
        dialogTitle.text = getString(R.string.enter_cvv_number)
        ...
        btnConfirmBtn.setOnClickListener {
            if (etPinEt.text.toString().isEmpty()) {
                Toast.makeText(context, getString(R.string.cvv_cannot_be_empty), Toast.LENGTH_SHORT)
                    .show()
                return@setOnClickListener
            }
            if (etPinEt.text.toString().length < 3) {
                Toast.makeText(context, getString(R.string.cvv_too_short), Toast.LENGTH_SHORT)
                    .show()
                return@setOnClickListener
            }
            enterCvvListener.onConfirm(etPinEt.text.toString().trim())
            dialog?.cancel()
            dialog?.dismiss()
            dismiss()
        }
