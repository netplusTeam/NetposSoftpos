...
public class BerTlvBuilder {
	...
	public BerTlvBuilder(BerTag aTemplate, byte[] aBuffer, int aOffset,
			int aLength) {
	...
	public int build() {
		...
		if (theTemplate != null) {
			...
			// shifts array
			System.arraycopy(theBuffer, theBufferOffset, theBuffer, tagLen
					+ lengthBytesCount, thePos);

			// copies tag
			System.arraycopy(theTemplate.bytes, 0, theBuffer, theBufferOffset,
					theTemplate.bytes.length);
	...
	private void fillLength(byte[] aBuffer, int aOffset, int aLength) {
		...
		if (aLength < 0x80) {
			...
			throw new IllegalStateException("length [" + aLength
					+ "] out of range (0x1000000)");
		}
	}
	...
	public BerTlvBuilder addBytes(BerTag aTag, byte[] aBytes, int aFrom,
			int aLength) {
	...
	public BerTlv buildTlv() {
		int count = build();
		return new BerTlvParser().parseConstructed(theBuffer, theBufferOffset,
				count);
	}
